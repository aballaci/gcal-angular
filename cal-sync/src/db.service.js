...
module.exports.fetchEventsToken = function fetchToken(cal_id) {
  SyncToken.findOne({cal_id: cal_id}, 'sync_token cal_id').then(
    token => {
      console.log("fetchEventsToken: " +  token);
      if(token === null) token = {cal_id: cal_id};
...
module.exports.fetchCalendarListToken = function fetchToken() {
  SyncToken.findOne({cal_id: "main"}, 'sync_token').then(
    token => {
...
module.exports.insertItems = function insertItems(calendarId, items) {
  ...
  items.forEach(function ( item ) {
    ...
    item.end = end;
    item.tags = item.description.match(/(^|\s)(#[A-Za-z0-9-äöüß]+)/gi);
    var eventSubType = item.description.match(/\=.*?\=/gi);
    if(eventSubType && eventSubType[0]) {
      item.eventSubType = eventSubType[0].replace(/=/g, '').replace(/ /g, '_');
    }
    item.description = item.description.replace(/(^|\s)(#[A-Za-z0-9-äöüß]+)/gi, '');
