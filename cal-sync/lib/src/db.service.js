...
module.exports.insertItems = function insertItems(calendarId, items) {
    ...
    items.forEach(function (item) {
        ...
        item.end = end;
        item.tags = item.description.match(/#[a-z]+/gi);
        var categories = item.description.match(/==[a-z]+/gi);
        if(categories) {
          item.categories = categories.map(x => x.replace('==', ''));
        }
        item.description = item.description.replace(/#[a-z]+/gi, '');
        item.description = item.description.replace(/==[a-z]+/gi, '');
        // Insert documents
